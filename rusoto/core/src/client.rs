...
use crate::credential::{CredentialsError, DefaultCredentialsProvider, ProvideAwsCredentials, StaticProvider};
        ...
        D: DispatchSignedRequest + Send + Sync + 'static,
{
    let f = client
        ...
        .as_ref()
        .ok_or_else(|| {
            SignAndDispatchError::Credentials(
                CredentialsError::new("credentials provider not specified")
            )
         })?.credentials();
