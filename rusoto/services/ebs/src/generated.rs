...
impl Ebs for EbsClient {
    ...
    async fn complete_snapshot(
        ...
        if let Some(ref checksum) = input.checksum {
            request.add_header("x-amz-Checksum", &checksum.to_string());
        }

        if let Some(ref checksum_aggregation_method) = input.checksum_aggregation_method {
            request.add_header(
                "x-amz-Checksum-Aggregation-Method",
                &checksum_aggregation_method.to_string(),
            );
        }

        if let Some(ref checksum_algorithm) = input.checksum_algorithm {
            request.add_header("x-amz-Checksum-Algorithm", &checksum_algorithm.to_string());
        }
    ...
    async fn put_snapshot_block(
        ...
        if let Some(ref progress) = input.progress {
            request.add_header("x-amz-Progress", &progress.to_string());
        }
