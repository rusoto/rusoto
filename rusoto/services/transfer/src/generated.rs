...
impl Into<String> for EndpointType {
    fn into(self) -> String {
        match self {
            ...
            EndpointType::Vpc => "VPC".to_string(),
            EndpointType::VpcEndpoint => "VPC_ENDPOINT".to_string(),
...
#[derive(Clone, Debug, Eq, PartialEq, Hash)]
pub struct UnknownHomeDirectoryType {
    name: String,
}
...
pub enum HomeDirectoryType {
    ...
    Path,
    #[doc(hidden)]
    UnknownVariant(UnknownHomeDirectoryType),
}

impl Default for HomeDirectoryType {
    fn default() -> Self {
        "".into()
    }
}

impl fmt::Display for HomeDirectoryType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(self.into())
    }
}

impl rusoto_core::param::ToParam for HomeDirectoryType {
    fn to_param(&self) -> String {
        self.to_string()
    }
}

impl Into<String> for HomeDirectoryType {
    fn into(self) -> String {
        match self {
            HomeDirectoryType::Logical => "LOGICAL".to_string(),
            HomeDirectoryType::Path => "PATH".to_string(),
            HomeDirectoryType::UnknownVariant(UnknownHomeDirectoryType { name: original }) => {
                original
            }
        }
    }
...
impl<'a> Into<&'a str> for &'a HomeDirectoryType {
    fn into(self) -> &'a str {
        match self {
...
impl From<&str> for HomeDirectoryType {
    fn from(name: &str) -> Self {
        match name {
            "LOGICAL" => HomeDirectoryType::Logical,
            "PATH" => HomeDirectoryType::Path,
            _ => HomeDirectoryType::UnknownVariant(UnknownHomeDirectoryType {
                name: name.to_owned(),
            }),
        }
    }
...
impl From<String> for HomeDirectoryType {
    fn from(name: String) -> Self {
        match &*name {
            "LOGICAL" => HomeDirectoryType::Logical,
            "PATH" => HomeDirectoryType::Path,
            _ => HomeDirectoryType::UnknownVariant(UnknownHomeDirectoryType { name }),
        }
    }
}
...
impl Serialize for HomeDirectoryType {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
...
impl<'de> Deserialize<'de> for HomeDirectoryType {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
...
/// <p>Returns information related to the type of user authentication that is in use for a file transfer protocol-enabled server's users. For <code>SERVICE_MANAGED</code> authentication, the Secure Shell (SSH) public keys are stored with a user on the server instance. For <code>API_GATEWAY</code> authentication, your custom authentication method is implemented by using an API call. The server can have only one method of authentication.</p>
...
impl Into<String> for IdentityProviderType {
    fn into(self) -> String {
        match self {
            IdentityProviderType::ApiGateway => "API_GATEWAY".to_string(),
            IdentityProviderType::ServiceManaged => "SERVICE_MANAGED".to_string(),
...
impl Into<String> for State {
    fn into(self) -> String {
        match self {
            State::Offline => "OFFLINE".to_string(),
            State::Online => "ONLINE".to_string(),
            State::Starting => "STARTING".to_string(),
            State::StartFailed => "START_FAILED".to_string(),
            State::Stopping => "STOPPING".to_string(),
            State::StopFailed => "STOP_FAILED".to_string(),
