...
pub trait KinesisFirehose {
    ...
    fn get_kinesis_stream(&self,
                          ...
                          -> RusotoFuture<GetKinesisStreamOutput, GetKinesisStreamError>;
          ...
          D: DispatchSignedRequest
{
        ...
        RusotoFuture::new({
                                ...
                                StatusCode::Ok => 
            {
                future::Either::A(response.body.concat2().map_err(|err| err.into()).map(|body| {
                    serde_json::from_str::<CreateDeliveryStreamOutput>(String::from_utf8_lossy(body.as_ref()).as_ref()).unwrap()
                }))
            },
                                _ => {
                                    future::Either::B(response.body.concat2().from_err().and_then(|body| {
                                        Err(CreateDeliveryStreamError::from_body(String::from_utf8_lossy(body.as_ref()).as_ref()))
                                    }))
                                }
                            }
                        })
    ...
    fn delete_delivery_stream
        ...
        RusotoFuture::new({
            ...
            {
                future::Either::A(response.body.concat2().map_err(|err| err.into()).map(|body| {
                    serde_json::from_str::<DeleteDeliveryStreamOutput>(String::from_utf8_lossy(body.as_ref()).as_ref()).unwrap()
                }))
            },
                                _ => {
                                    future::Either::B(response.body.concat2().from_err().and_then(|body| {
                                        Err(DeleteDeliveryStreamError::from_body(String::from_utf8_lossy(body.as_ref()).as_ref()))
                                    }))
                                }
                            }
                        })
    ...
    fn describe_delivery_stream
        ...
        RusotoFuture::new({
            ...
            {
                future::Either::A(response.body.concat2().map_err(|err| err.into()).map(|body| {
                    serde_json::from_str::<DescribeDeliveryStreamOutput>(String::from_utf8_lossy(body.as_ref()).as_ref()).unwrap()
                }))
            },
                                _ => {
                                    future::Either::B(response.body.concat2().from_err().and_then(|body| {
                                        Err(DescribeDeliveryStreamError::from_body(String::from_utf8_lossy(body.as_ref()).as_ref()))
                                    }))
                                }
                            }
                        })
    ...
    fn get_kinesis_stream(&self,
        ...
        RusotoFuture::new({
            ...
            {
                future::Either::A(response.body.concat2().map_err(|err| err.into()).map(|body| {
                    serde_json::from_str::<GetKinesisStreamOutput>(String::from_utf8_lossy(body.as_ref()).as_ref()).unwrap()
                }))
            },
                                _ => {
                                    future::Either::B(response.body.concat2().from_err().and_then(|body| {
                                        Err(GetKinesisStreamError::from_body(String::from_utf8_lossy(body.as_ref()).as_ref()))
                                    }))
                                }
                            }
                        })
    ...
    fn list_delivery_streams
        ...
        RusotoFuture::new({
            ...
            {
                future::Either::A(response.body.concat2().map_err(|err| err.into()).map(|body| {
                    serde_json::from_str::<ListDeliveryStreamsOutput>(String::from_utf8_lossy(body.as_ref()).as_ref()).unwrap()
                }))
            },
                                _ => {
                                    future::Either::B(response.body.concat2().from_err().and_then(|body| {
                                        Err(ListDeliveryStreamsError::from_body(String::from_utf8_lossy(body.as_ref()).as_ref()))
                                    }))
                                }
                            }
                        })
                                  ...
                                  .and_then(|response| match response.status {
                                                StatusCode::Ok => {
                                                    future::Either::A(response
                                                                          ...
                                                                          .map(|body| {
                serde_json::from_str::<PutRecordOutput>(String::from_utf8_lossy(body.as_ref())
                                                            .as_ref())
                        .unwrap()
            }))
                                                }
                                                _ => {
                                                    future::Either::B(response
                                                                          ...
                                                                          .from_err()
                                                                          .and_then(|body| {
                Err(PutRecordError::from_body(String::from_utf8_lossy(body.as_ref()).as_ref()))
            }))
                                                }
                                            })
                          })
                                  ...
                                  .and_then(|response| match response.status {
                                                StatusCode::Ok => {
                                                    future::Either::A(response
                                                                          ...
                                                                          .map(|body| {
                serde_json::from_str::<PutRecordBatchOutput>(String::from_utf8_lossy(body.as_ref())
                                                                 .as_ref())
                        .unwrap()
            }))
                                                }
                                                _ => {
                                                    future::Either::B(response
                                                                          ...
                                                                          .from_err()
                                                                          .and_then(|body| {
                Err(PutRecordBatchError::from_body(String::from_utf8_lossy(body.as_ref()).as_ref()))
            }))
                                                }
                                            })
                          })
    ...
    fn update_destination(&self,
        ...
        RusotoFuture::new({
            ...
            {
                future::Either::A(response.body.concat2().map_err(|err| err.into()).map(|body| {
                    serde_json::from_str::<UpdateDestinationOutput>(String::from_utf8_lossy(body.as_ref()).as_ref()).unwrap()
                }))
            },
                                _ => {
                                    future::Either::B(response.body.concat2().from_err().and_then(|body| {
                                        Err(UpdateDestinationError::from_body(String::from_utf8_lossy(body.as_ref()).as_ref()))
                                    }))
                                }
                            }
                        })
