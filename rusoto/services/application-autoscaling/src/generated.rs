...
impl Into<String> for AdjustmentType {
    fn into(self) -> String {
        match self {
            AdjustmentType::ChangeInCapacity => "ChangeInCapacity".to_string(),
            AdjustmentType::ExactCapacity => "ExactCapacity".to_string(),
            AdjustmentType::PercentChangeInCapacity => "PercentChangeInCapacity".to_string(),
...
#[derive(Clone, Debug, Eq, PartialEq, Hash)]
pub struct UnknownMetricAggregationType {
    name: String,
}
...
pub enum MetricAggregationType {
    ...
    Minimum,
    #[doc(hidden)]
    UnknownVariant(UnknownMetricAggregationType),
}

impl Default for MetricAggregationType {
    fn default() -> Self {
        "".into()
    }
}

impl fmt::Display for MetricAggregationType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(self.into())
    }
}

impl rusoto_core::param::ToParam for MetricAggregationType {
    fn to_param(&self) -> String {
        self.to_string()
    }
}
...
impl Into<String> for MetricAggregationType {
    fn into(self) -> String {
        match self {
            MetricAggregationType::Average => "Average".to_string(),
            MetricAggregationType::Maximum => "Maximum".to_string(),
            MetricAggregationType::Minimum => "Minimum".to_string(),
            MetricAggregationType::UnknownVariant(UnknownMetricAggregationType {
                name: original,
            }) => original,
        }
    }
...
impl<'a> Into<&'a str> for &'a MetricAggregationType {
    fn into(self) -> &'a str {
        match self {
...
impl From<String> for MetricAggregationType {
    fn from(name: String) -> Self {
        match &*name {
            "Average" => MetricAggregationType::Average,
            "Maximum" => MetricAggregationType::Maximum,
            "Minimum" => MetricAggregationType::Minimum,
            _ => MetricAggregationType::UnknownVariant(UnknownMetricAggregationType { name }),
        }
    }
}
    ...
    fn into(self) -> String {
        match self {
            ...
            MetricType::AppStreamAverageCapacityUtilization => {
                "AppStreamAverageCapacityUtilization".to_string()
            }
            MetricType::CassandraReadCapacityUtilization => {
                "CassandraReadCapacityUtilization".to_string()
            }
            MetricType::CassandraWriteCapacityUtilization => {
                "CassandraWriteCapacityUtilization".to_string()
            }
            MetricType::ComprehendInferenceUtilization => {
                "ComprehendInferenceUtilization".to_string()
            }
            ...
            MetricType::KafkaBrokerStorageUtilization => {
                "KafkaBrokerStorageUtilization".to_string()
            }
            MetricType::LambdaProvisionedConcurrencyUtilization => {
                "LambdaProvisionedConcurrencyUtilization".to_string()
            }
            ...
            MetricType::SageMakerVariantInvocationsPerInstance => {
                "SageMakerVariantInvocationsPerInstance".to_string()
            }
...
impl Into<String> for PolicyType {
    fn into(self) -> String {
        match self {
            PolicyType::StepScaling => "StepScaling".to_string(),
    ...
    fn into(self) -> String {
        match self {
            ScalableDimension::AppstreamFleetDesiredCapacity => {
                "appstream:fleet:DesiredCapacity".to_string()
            }
            ScalableDimension::CassandraTableReadCapacityUnits => {
                "cassandra:table:ReadCapacityUnits".to_string()
            }
            ScalableDimension::CassandraTableWriteCapacityUnits => {
                "cassandra:table:WriteCapacityUnits".to_string()
            }
            ScalableDimension::ComprehendDocumentClassifierEndpointDesiredInferenceUnits => {
                "comprehend:document-classifier-endpoint:DesiredInferenceUnits".to_string()
            }
            ScalableDimension::ComprehendEntityRecognizerEndpointDesiredInferenceUnits => {
                "comprehend:entity-recognizer-endpoint:DesiredInferenceUnits".to_string()
            }
            ScalableDimension::CustomResourceResourceTypeProperty => {
                "custom-resource:ResourceType:Property".to_string()
            }
            ...
            ScalableDimension::ElasticmapreduceInstancegroupInstanceCount => {
                "elasticmapreduce:instancegroup:InstanceCount".to_string()
            }
            ScalableDimension::KafkaBrokerStorageVolumeSize => {
                "kafka:broker-storage:VolumeSize".to_string()
            }
            ScalableDimension::LambdaFunctionProvisionedConcurrency => {
                "lambda:function:ProvisionedConcurrency".to_string()
            }
            ...
            ScalableDimension::SagemakerVariantDesiredInstanceCount => {
                "sagemaker:variant:DesiredInstanceCount".to_string()
            }
...
impl Into<String> for ScalingActivityStatusCode {
    fn into(self) -> String {
        match self {
            ...
            ScalingActivityStatusCode::Overridden => "Overridden".to_string(),
            ScalingActivityStatusCode::Pending => "Pending".to_string(),
            ScalingActivityStatusCode::Successful => "Successful".to_string(),
            ScalingActivityStatusCode::Unfulfilled => "Unfulfilled".to_string(),
...
impl Into<String> for ServiceNamespace {
    fn into(self) -> String {
        match self {
            ServiceNamespace::Appstream => "appstream".to_string(),
            ServiceNamespace::Cassandra => "cassandra".to_string(),
            ServiceNamespace::Comprehend => "comprehend".to_string(),
            ServiceNamespace::CustomResource => "custom-resource".to_string(),
            ServiceNamespace::Dynamodb => "dynamodb".to_string(),
            ServiceNamespace::Ec2 => "ec2".to_string(),
            ServiceNamespace::Ecs => "ecs".to_string(),
            ServiceNamespace::Elasticmapreduce => "elasticmapreduce".to_string(),
            ServiceNamespace::Kafka => "kafka".to_string(),
            ServiceNamespace::Lambda => "lambda".to_string(),
            ServiceNamespace::Rds => "rds".to_string(),
            ServiceNamespace::Sagemaker => "sagemaker".to_string(),
...
impl From<&str> for ServiceNamespace {
    fn from(name: &str) -> Self {
        match name {
            ...
            "elasticmapreduce" => ServiceNamespace::Elasticmapreduce,
