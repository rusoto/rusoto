...
impl Route53 for Route53Client {
    ...
    async fn list_hosted_zones_by_vpc(
        ...
        if !response.status.is_success() {
            let response = response.buffer().await.map_err(RusotoError::HttpDispatch)?;
            return Err(ListHostedZonesByVPCError::from_response(response));
        }

        let mut result;
        let xml_response = response.buffer().await.map_err(RusotoError::HttpDispatch)?;
        if xml_response.body.is_empty() {
            result = ListHostedZonesByVPCResponse::default();
        } else {
            let reader = EventReader::new_with_config(
                xml_response.body.as_ref(),
                ParserConfig::new().trim_whitespace(false),
            ...
            let _start_document = stack.next();
            let actual_tag_name = peek_at_name(&mut stack)?;
            result = ListHostedZonesByVPCResponseDeserializer::deserialize(
                &actual_tag_name,
                &mut stack,
            )?;
        }
