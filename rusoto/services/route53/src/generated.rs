...
pub struct ResourceRecordSet {
    ...
    pub multi_value_answer: Option<ResourceRecordSetMultiValueAnswer>,
...
pub type ResourceRecordSetMultiValueAnswer = bool;
...
impl ResourceRecordSetMultiValueAnswerDeserializer {
    ...
    fn deserialize<'a, T: Peek + Next>
        (tag_name: &str,
         stack: &mut T)
         -> Result<ResourceRecordSetMultiValueAnswer, XmlParseError> {
        try!(start_element(tag_name, stack));
        let obj = bool::from_str(try!(characters(stack)).as_ref()).unwrap();
        try!(end_element(tag_name, stack));
...
impl ResourceRecordSetMultiValueAnswerSerializer {
    #[allow(unused_variables, warnings)]
    pub fn serialize(name: &str, obj: &ResourceRecordSetMultiValueAnswer) -> String {
        format!("<{name}>{value}</{name}>",
                name = name,
                value = obj.to_string())
    }
