...
impl ComputeOptimizer for ComputeOptimizerClient {
    ...
    > {
        ...
        if response.status.is_success() {
            ...
            Err(DescribeRecommendationExportJobsError::from_response(
                response,
            ))
        }
    }
    ...
    > {
        ...
        if response.status.is_success() {
            ...
            let try_response = response.buffer().await;
            let response = try_response.map_err(RusotoError::HttpDispatch)?;
            Err(ExportAutoScalingGroupRecommendationsError::from_response(
                response,
            ))
        }
    }
    ...
    > {
        ...
        if response.status.is_success() {
            ...
            let try_response = response.buffer().await;
            let response = try_response.map_err(RusotoError::HttpDispatch)?;
            Err(ExportEC2InstanceRecommendationsError::from_response(
                response,
            ))
        }
    }
