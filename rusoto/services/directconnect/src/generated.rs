...
impl DirectConnect for DirectConnectClient {
    ...
    > {
        ...
        if response.status.is_success() {
            ...
            let try_response = response.buffer().await;
            let response = try_response.map_err(RusotoError::HttpDispatch)?;
            Err(ListVirtualInterfaceTestHistoryError::from_response(
                response,
            ))
        }
    }
    ...
    async fn start_bgp_failover_test(
        ...
        if response.status.is_success() {
            ...
            let try_response = response.buffer().await;
            let response = try_response.map_err(RusotoError::HttpDispatch)?;
            Err(StartBgpFailoverTestError::from_response(response))
        }
    }
    ...
    async fn stop_bgp_failover_test(
        ...
        if response.status.is_success() {
            let response = response.buffer().await.map_err(RusotoError::HttpDispatch)?;
            proto::json::ResponsePayload::new(&response)
                .deserialize::<StopBgpFailoverTestResponse, _>()
        } else {
            let try_response = response.buffer().await;
            let response = try_response.map_err(RusotoError::HttpDispatch)?;
            Err(StopBgpFailoverTestError::from_response(response))
        }
    }
