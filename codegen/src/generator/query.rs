...
fn generate_list_serializer(service: &Service, shape: &Shape) -> String {
    ...
    parts.push(
        format!("for (index, obj) in obj.iter().enumerate() {{
...
fn value_name(service: &Service, shape: &Shape) -> String {
    let value_name = shape.value.as_ref()
...
fn member_location(service: &Service, member: &Member, default: &str) -> String {
    let member_location = member.location_name.clone().unwrap_or(default.to_owned());
...
fn generate_struct_field_serializers(service: &Service, shape: &Shape) -> String {
    shape.members.as_ref().unwrap().iter().map(|(member_name, member)| {
        ...
        match shape.required(member_name) {
            true => {
                match primitive {
                    true => required_primitive_field_serializer(service, member_name, member),
                    false => required_complex_field_serializer(service, member_name, member)
                }
            },
            false => {
                match primitive {
                    true => optional_primitive_field_serializer(service, member_name, member),
                    false => optional_complex_field_serializer(service, member_name, member)
                }
            }
        }
    }).collect::<Vec<String>>().join("\n")
...
fn optional_primitive_field_serializer(service: &Service, member_name: &str, member: &Member) -> String {
...
fn required_primitive_field_serializer(service: &Service, member_name: &str, member: &Member) -> String {
    let member_shape = service.shape_for_member(&member).unwrap();
...
fn serialize_primitive_expression(shape_type: &ShapeType, var_name: &str) -> String {
    match *shape_type {
        ...
        ShapeType::Integer | ShapeType::Double | ShapeType::Long | ShapeType::Boolean => format!("&{}.to_string()", var_name),
        ShapeType::Blob => format!("::std::str::from_utf8(&{}).unwrap()", var_name),
        shape_type => panic!("Unknown primitive shape type: {:?}", shape_type),
    }
}

fn required_complex_field_serializer(service: &Service, member_name: &str, member: &Member) -> String {
...
fn optional_complex_field_serializer(service: &Service, member_name: &str, member: &Member) -> String {
    format!(
        "if let Some(ref field_value) = obj.{field_name} {{
