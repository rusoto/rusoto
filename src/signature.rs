...
impl<'a> SignedRequest<'a> {
    ...
    pub fn sign_and_execute(&mut self, creds: &AWSCredentials) -> Response {
        ...
        let scope = format!("{}/{}/{}/aws4_request",
                            ...
                            region_in_aws_format(&self.region),
                            &self.service);
        ...
        let signing_key = signing_key(&creds.get_aws_secret_key(),
                                      date,
                                      &region_in_aws_format(&self.region),
                                      &self.service);
        ...
        let auth_header = format!("AWS4-HMAC-SHA256 Credential={}/{}, SignedHeaders={}, \
                                   Signature={}",
...
fn extract_s3_redirect_location(response: Response) -> Result<String, AWSError> {
    ...
    if response.status != HTTP_TEMPORARY_REDIRECT {
        return Err(AWSError::new("Trying to find temporary location when status is not 307 temp \
                                  redirect."));
    }
...
fn field_in_s3_redirect(name: &str) -> bool {
    if name == "Code" || name == "Message" || name == "Bucket" || name == "RequestId" ||
       name == "HostId" {
        return true;
    }
    false
...
fn extract_s3_temporary_endpoint_from_xml<'a, T: Peek + Next>(stack: &mut T)
