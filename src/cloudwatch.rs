	...
	fn should_serialize_complex_metric_data_params() {
		let mock = MockRequestDispatcher::with_status(200)
            ...
            .with_request_checker(|request: &SignedRequest| {
                ...
                assert_eq!(request.params.get("Namespace"), Some(&Some("TestNamespace".to_owned())));                
                assert_eq!(request.params.get("MetricData.member.1.MetricName"), Some(&Some("buffers".to_owned())));
                assert_eq!(request.params.get("MetricData.member.1.Unit"), Some(&Some("Bytes".to_owned())));
                assert_eq!(request.params.get("MetricData.member.1.Value"), Some(&Some("1".to_owned())));
                assert_eq!(request.params.get("MetricData.member.1.Dimensions.member.1.Name"), Some(&Some("foo".to_owned())));
                assert_eq!(request.params.get("MetricData.member.1.Dimensions.member.1.Value"), Some(&Some("bar".to_owned())));                
