...
//! 3. IAM instance profile.  Will only work if running on an EC2 instance with
...
/// Represents AWS credentials.  Includes access key, secret key, token (for IAM profiles)
...
impl AWSCredentialsProvider for EnvironmentCredentialsProvider {
    fn get_credentials(&mut self) -> Result<&AWSCredentials, AWSError> {
        if self.credentials.is_none() ||
           self.credentials.as_ref().unwrap().credentials_are_expired() {
...
fn get_credentials_from_environment<'a>() -> Result<AWSCredentials, AWSError> {
    ...
    if env_key.is_empty() || env_secret.is_empty() {
        return Err(AWSError::new("Couldn't find either AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY \
                                  or both in environment."));
    }
    ...
    for line in file_lines.lines() {
        ...
        if profile_regex.is_match(&unwrapped_line) {
            ...
            if profile_name.is_some() && access_key.is_some() && secret_key.is_some() {
                let creds = AWSCredentials::new(access_key.unwrap(),
                                                secret_key.unwrap(),
                                                None,
                                                in_ten_minutes());
    ...
    fn get_credentials(&mut self) -> Result<&AWSCredentials, AWSError> {
        if self.credentials.is_none() ||
           self.credentials.as_ref().unwrap().credentials_are_expired() {
            ...
            let mut address: String = "http://169.254.169.\
                                       254/latest/meta-data/iam/security-credentials"
                                          .to_string();
            ...
            match client.get(&address)
                        ...
                        .send() {
                Err(_) => {
                    return err("Didn't get a parseable response body from instance role details")
                }
                Ok(received_response) => response = received_response,
            };
            ...
            match json_object.find("Token") {
                ...
                Some(val) => {
                    token_from_response = val.as_string()
            ...
            self.credentials = Some(AWSCredentials::new(access_key,
                                                        secret_key,
        ...
        if self.credentials.is_none() ||
           self.credentials.as_ref().unwrap().credentials_are_expired() {
            ...
            let profile_credentials_provider_result = match self.file_path {
                Some(ref file_path) => {
                    Ok(ProfileCredentialsProvider::with_configuration(&self.profile, file_path))
                }
            ...
            return Err(AWSError::new("Couldn't find AWS credentials in environment, default \
                                      credential file location or IAM role."));
        }
...
impl DefaultAWSCredentialsProviderChain {
    ...
    pub fn with_configuration<'a>(profile: &'a str,
                                  file_path: &'a str)
                                  -> DefaultAWSCredentialsProviderChain {
        DefaultAWSCredentialsProviderChain {
            credentials: None,
    ...
    pub fn set_file_path<S>(&mut self, file_path: S)
        where S: Into<String>
    {
    ...
    pub fn set_profile<S>(&mut self, profile: S)
        where S: Into<String>
    {
...
mod tests {
    ...
    fn parse_credentials_file_multiple_profiles() {
        let result = super::parse_credentials_file("tests/sample-data/multiple_profile_credential\
                                                    s");
        assert!(result.is_ok());
    ...
    fn profile_credentials_provider_happy_path() {
        let mut provider = ProfileCredentialsProvider::with_configuration("foo",
                                                                          "tests/sample-data/mult\
                                                                           iple_profile_credentia\
                                                                           ls");
    ...
    fn profile_credentials_provider_bad_profile() {
        let mut provider = ProfileCredentialsProvider::with_configuration("not_a_profile",
                                                                          "tests/sample-data/mult\
                                                                           iple_profile_credentia\
                                                                           ls");
        let result = provider.get_credentials();
        ...
        assert_eq!(result.err(),
                   Some(AWSError("profile not found".to_string())));
    }
    ...
    fn credential_chain_with_configuration() {
        let mut chain = DefaultAWSCredentialsProviderChain::with_configuration("foo",
                                                                               "tests/sample-data\
                                                                                /multiple_profile\
                                                                                _credentials");

        let credentials = chain.get_credentials()
                               .expect("Failed to get credentials from default provider chain \
                                        with configuration");
    ...
    fn parse_credentials_bad_path() {
        ...
        assert_eq!(result.err(),
                   Some(AWSError::new("Couldn't stat credentials file.")));
    }
