...
//! # Examples
//!
//! ## Assume role and get session credentials
//!
//! ```
...
//! let sts_client = StsClient::new(default_tls_client().unwrap(), base_provider, region);
...
//!                         role_session_name: "my_session".to_owned(),
//!                         ..Default::default()
//!                     })
//!         .map_err(|e| "assume role error")
//!         .and_then(|response| {
//!             AwsCredentials::new_for_credentials(response.credentials.unwrap())
//!                 .map_err(|e| "new_for_credentials error")
//!         });
//! ```
//!
//! ## Assume role and create a credentials provider for use with other Rusoto AWS Api clients.
//!
//! ```
